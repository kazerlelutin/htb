...
<header class="site-header"><a class="wordmark" href="/?lang={{.Language}}" aria-label="HTB home"><span aria-hidden="true">&gt;</span><span class="cursor" aria-hidden="true">_</span> HTB</a><nav aria-label="{{if eq .Language "fr"}}Navigation principale{{else}}Main navigation{{end}}"><a href="/commands?lang={{.Language}}">{{if eq .Language "fr"}}Commandes{{else}}Commands{{end}}</a><a href="/downloads?lang={{.Language}}">{{if eq .Language "fr"}}Télécharger{{else}}Download{{end}}</a><a href="/cgu?lang={{.Language}}">{{if eq .Language "fr"}}CGU{{else}}Terms{{end}}</a>{{if .BrowserLogin}}<a href="/login">{{if eq .Language "fr"}}Espace client{{else}}Client space{{end}}</a>{{end}}</nav><nav class="languages" aria-label="Language"><a href="{{.FrenchURL}}" lang="fr" hreflang="fr">FR</a><span aria-hidden="true">/</span><a href="{{.EnglishURL}}" lang="en" hreflang="en">EN</a></nav></header>
