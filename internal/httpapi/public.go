...
func (s *Server) home(w http.ResponseWriter, r *http.Request) {
	...
	language := publicLanguage(r)
	body := homeBody(language) + deploymentModesBody(language, s.browserLogin != nil, s.publicURL) + simulatorBody(language)
...
func legalNoticeBody(language string) template.HTML {
	if language == "en" {
		return template.HTML(`<article class="legal"><p class="eyebrow">LEGAL NOTICE</p><h1>Legal notice</h1><h2>Publisher</h2><p>HTB (Headless Ticket Board) is published by Kazer Lelutin. Contact: <a href="mailto:b@bouteiller.contact">b@bouteiller.contact</a>.</p><h2>Hosting</h2><p>The service is deployed by Ben-to on a CapRover-managed infrastructure. The service domain is <a href="https://htboard.xyz">htboard.xyz</a>.</p><h2>Intellectual property</h2><p>The HTB name, source code, content, and visual identity may not be reproduced outside the applicable rights and licences without the publisher’s prior permission.</p><h2>Liability</h2><p>The publisher makes reasonable efforts to keep the service available and information accurate, but cannot guarantee uninterrupted or error-free operation.</p></article>`)
	}
	return template.HTML(`<article class="legal"><p class="eyebrow">MENTIONS LÉGALES</p><h1>Mentions légales</h1><h2>Éditeur</h2><p>HTB (Headless Ticket Board) est édité par Kazer Lelutin. Contact : <a href="mailto:b@bouteiller.contact">b@bouteiller.contact</a>.</p><h2>Hébergement</h2><p>Le service est déployé par Ben-to sur une infrastructure administrée avec CapRover. Le domaine du service est <a href="https://htboard.xyz">htboard.xyz</a>.</p><h2>Propriété intellectuelle</h2><p>Le nom HTB, le code source, les contenus et l’identité visuelle ne peuvent être reproduits hors des droits et licences applicables sans l’autorisation préalable de l’éditeur.</p><h2>Responsabilité</h2><p>L’éditeur met en œuvre des moyens raisonnables pour assurer l’exactitude des informations et la disponibilité du service, sans garantir une disponibilité ininterrompue ni l’absence d’erreur.</p></article>`)
}
